...
Bound everything: the DTN store (recipient count, store-wide packet total, age — deliberately NOT a per-recipient depth, which silently dropped a busy peer's oldest packets while the store sat empty and pinned confirmation custody at the cap) and per-neighbor relay rate are all capped — an unbounded flood/cache is an abuse and battery sink. The bound applies to every buffer on the message path, not just the sealed store: the **pre-seal hold** (messages waiting for a first session, which retain full plaintext payloads) and the **custody packetId index** (entries leave on ACK, so anything never ACKed would accumulate) are capped too. Capping the sealed store while an unsealed buffer in front of it grows without limit just moves the leak upstream. ANNOUNCE is **not** flooded; it stays neighbor-local (presence, not reach).
...
- **Everything that can be encrypted is encrypted.** Only ANNOUNCE and the Noise handshake travel in the clear — every other packet is a sealed `secure` envelope, *including* the sync-on-connect custody exchange (offer/request ride as `SecureFrame` content types sealed to the peer's session — TTL 1 on BLE, direct on UDP; the UDP offer lists only packets addressed to that peer, since UDP is point-to-point and carries no relay custody). A custody inventory is not put on the air just because the packetIds also appear in flood headers: the session already exists when sync runs, so there is no reason to transmit plaintext.
