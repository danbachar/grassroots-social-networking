...
Everything that survives the early returns — i.e.\ every \code{secure} envelope — is a mesh packet. The router deduplicates it by calling \code{checkAndAdd} on the outer \code{packet.packetId} against a \code{BloomFilter} of recently-seen IDs; \code{firstSeen} is the negation of ``was already present''. Before anything else, note the outermost branch: the whole relay-and-carry block is nested inside \code{if (!forUs)}. A packet addressed to \emph{this} node is never relayed no matter what its TTL says — it is deduplicated, trial-decrypted and delivered, and that is all. Relaying is what happens to \emph{other people's} traffic. Within that branch, a packet is relayed only when two conditions hold simultaneously:

\begin{enumerate}
\item \textbf{First sight.} \code{firstSeen} is true — a duplicate is dropped, killing loops and re-floods.
\item \textbf{TTL budget.} \code{packet.ttl > 0}. The relay forwards \code{packet.decrementTtl()}, so the TTL falls by one per arrival and a packet arriving at~0 is not forwarded. A packet arriving at~1 still gets its last hop, going out at~0 --- worth making because the destination is exempt from this branch entirely, so a neighbour who is the recipient still accepts it. The default starting TTL is~7 (\code{packet.dart}), bounding any packet to at most seven hops of spread.
\end{enumerate}
...
The dedup filter (\code{lib/src/mesh/bloom\_filter.dart}) is 96{,}000 bits with 7 hash functions (double-hashed: FNV-1a plus a Murmur3-like second hash). It self-rotates by a \textbf{hard full clear} — not a generational scheme — when either the item count reaches 10{,}000 or 5~minutes elapse since the last rotation (\code{bloom\_filter.dart}). The consequence worth naming honestly: a packet ID seen just before a rotation can be treated as new just after it, so the same packet could in principle be re-processed or re-relayed across a rotation boundary. This is an accepted trade-off — the filter must be bounded in both memory and staleness, and TTL caps the damage of any re-flood. ANNOUNCE and handshakes never touch this filter at all, having returned early.

\subsection*{Bounding the flood: TTL and dedup, and nothing else}

An open flood needs a bound, and it has exactly two: a TTL that is decremented once per arrival and refuses to forward at~0, and a \code{BloomFilter} of seen packet IDs that relays any given packet at most once. Together they cap a packet's spread at seven hops and one transmission per node per packet, which is a bound on the total work the flood can create --- not merely on its rate.
...
Since unverified relaying cannot be bounded by cryptography, it is bounded by \emph{resource limits} instead. Two mechanisms cap abuse: a TTL that strictly decrements once per arrival and refuses to forward at $\text{ttl} = 0$; and a \code{BloomFilter} of seen packet IDs that suppresses loops and re-sends (\code{lib/src/routing/message_router.dart}). A third --- a per-neighbour relay rate cap --- was removed after it was measured shaping delivery on an ordinary eight-phone mesh rather than bounding abuse; see \emph{Bounding the flood}. The two that remain bound the total work a packet can create, but not the rate at which a neighbour can create it.
