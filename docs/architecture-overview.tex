...
An outbound message is sealed to the recipient's Noise session and, on BLE, flooded to all neighbors; the send succeeds as soon as those sealed packets are in the sender's own DTN memory buffer — the flood count decides only whether the message is shown as \emph{sent} or stays \emph{queued}, because packets nobody was in range to hear are still held for the next sync exchange. If no session exists yet the plaintext waits in a bounded pre-seal hold until the eager handshake on the next pairing makes sealing possible. There is no separate retry queue and no ACK-timeout re-send: redelivery happens exclusively through the sync exchange each time a pairing forms (see \emph{Mesh Routing}). Inbound, a peer is only reported \emph{connected} once an authenticated Noise session exists — reachability is gated on cryptographic authentication, not on a mere radio contact (\code{lib/src/grassroots_network.dart}). Presence and identity ride on a separate, non-relayed channel: the self-signed ANNOUNCE, a neighbor-local broadcast (TTL 1) whose payload carries the full public key, nickname, and an Ed25519 signature over that body (\code{lib/src/grassroots_network.dart}). ANNOUNCE is the \emph{only} packet signed in the clear on the wire; all message, ack, read-receipt, and signaling traffic authenticates end-to-end by decrypting under a verified Noise session, never by a per-packet signature. (One signed object does travel sealed inside that traffic — the inviter-signed invite blob an invitee presents in an \code{INTRODUCE} — but it is a capability that authorizes first contact, not a packet authenticator; see \emph{Facilitators}.)

\subsection{Roadmap}

The remainder of this chapter builds outward from these foundations. \emph{Identity, Trust \& the Security Model} develops the key-pair identity, the sender-anonymous envelope, and why relaying is unauthenticated by construction. \emph{Wire Format, Packets \& the Data Model} specifies the 54-byte header and the sealed \code{SecureFrame} that now carries content type and fragmentation inside the ciphertext. \emph{Transport Abstraction \& the BLE Mesh Transport} covers the common interface and the mandatory dual-role BLE design, and \emph{The Internet Transport \& NAT Traversal} covers the UDX path, address candidates, and hole-punching. \emph{Mesh Routing} details managed flooding, the \code{BloomFilter} dedup, the DTN memory buffer that makes store-carry-forward work, and its sealed sync exchange. \emph{The Noise Session Layer} covers the \code{Noise_XX} handshake, trial-decrypt, and the AAD construction. \emph{State Management} explains the Redux projection, and \emph{Instrumentation} describes the tracing and testbed machinery that produces the measured numbers this chapter relies on. Later sections walk through end-to-end interactions and close with design decisions, trade-offs, and future work.

\subsection{Lineage and a Warning about Stale Documentation}

The transport descends from the Bitchat protocol. That lineage is all but retired in the repository --- the README describes the current system, and a set of Bitchat-era agent instructions that still forbade relaying and store-and-forward outright has been deleted (one dead artefact, \code{lib/src/proto/bitchat.proto}, is still on disk, referenced by nothing) --- but it is worth recording, because it explains why so much of this design reads as a deliberate reversal: the ancestor delivered only to peers in direct range and never forwarded on anyone's behalf.
...
\code{ContentType} has \textbf{eight} values, and it is worth listing all of them because the sealed envelope is the only place they appear:
...
0x05 syncOffer         sync-on-connect: the packetIds this node's DTN store holds
0x06 syncRequest       reply to an offer: the subset the peer wants conveyed
0x07 testbedStart      DEBUG/TESTBED: remote start for a scripted field run
0x08 testbedNeighbours DEBUG/TESTBED: gossiped one-hop neighbour list, armed-only
\end{verbatim}

The last four matter to the architecture, not just to the enum. \code{syncOffer}/\code{syncRequest} carry the entire store-carry-forward reconciliation (\emph{Mesh Routing}), which is why the sync exchange is described there as sealed rather than cleartext. The two testbed types exist so a field run can be started across a dispersed fleet and its mesh size checked before starting; they are sealed like everything else, so only a peer holding a Noise session can trigger either, and the receiver additionally ignores \code{testbedStart} unless it has been armed with a matching experiment id (\emph{Instrumentation}).
...
\midrule
0  & 1  & content type: one of eight (message, ack, readReceipt, signaling, \\
   &    & \quad syncOffer, syncRequest, testbedStart, testbedNeighbours) \\
1  & 2  & fragment index \\
...
Crucially, a relay forwards the packet's bytes \emph{unchanged}. The router hands the decremented packet to the coordinator's \code{onRelay} callback (\code{grassroots\_network.dart}), which re-floods over BLE only --- UDP stays direct point-to-point, per \emph{The Internet Transport} --- via \code{\_floodViaBle(packet.serialize())} and thence \code{BleTransportService.broadcast(bytes, excludePeerIds)}, writing that one identical, already-sealed ciphertext to every ready BLE neighbour. A relay \emph{cannot} re-seal, because it cannot decrypt; it copies opaque bytes. This is distinct from the coordinator's own \code{broadcast(payload)} method, which does seal a \emph{fresh} copy per neighbour under each peer's Noise session --- but that path sends plaintext the coordinator itself holds, and is not relaying. On the relay path the inbound peer is excluded from the fan-out (\code{excludeBlePeerId}, \code{message\_router.dart}) so a packet is never echoed back at the neighbour it arrived from. That exclusion is resolved to a peer \emph{identity}, not to a path id: a converged pair holds two GATT legs, and excluding only the leg a packet arrived on let the pair's other leg echo it straight back.

For the same reason, a flood writes each packet to exactly \textbf{one leg per peer}, preferring our peripheral leg --- a notification is unacknowledged at ATT level and several can pack into one connection interval, whereas the central leg writes to the peer's characteristic. Writing both legs put identical bytes on the air twice per neighbour and the receiver's packetId Bloom simply dropped the second copy. This was measured rather than reasoned about: a 500-message cycle-check run recorded exactly 1000 redundant packet arrivals --- a clean 2$\times$ --- before the change, and 0 after. The second leg is still maintained --- it is the pair's other direction, and it is now literally the redundancy on failure: when the chosen leg refuses a write, \code{\_writeToPeer} retries the same bytes on \code{otherLegFor}'s leg and logs a \code{retry} trace record for the save. This is not the double-send the one-leg rule forbids, because the fallback runs only after a throw and both platforms fail \emph{before} transmitting, so the packet cannot reach the air twice. The retry is gated on the other leg's own MTU (the two legs negotiate separately, and writing a packet that leg will truncate would put an unparseable write on the air and report success) and is skipped for a path whose identity is unknown, since without a pubkey there is no way to tell the pair's other leg from a stranger's.

\subsection*{The Bloom filter and its hard-clear rotation}

The dedup filter (\code{lib/src/mesh/bloom\_filter.dart}) is 96{,}000 bits with 7 hash functions (double-hashed: FNV-1a plus a Murmur3-like second hash). It self-rotates by a \textbf{hard full clear} — not a generational scheme — when either the item count reaches 10{,}000 or 5~minutes elapse since the last rotation (\code{bloom\_filter.dart}). The consequence worth naming honestly: a packet ID seen just before a rotation can be treated as new just after it, so the same packet could in principle be re-processed or re-relayed across a rotation boundary. This is an accepted trade-off — the filter must be bounded in both memory and staleness, and TTL plus the rate budget cap the damage of any re-flood. ANNOUNCE and handshakes never touch this filter at all, having returned early.

\subsection*{Bounding the flood: the per-neighbour relay budget}

An open flood without a rate cap is a battery sink and an abuse vector. \code{\_maxRelaysPerWindow} is 512 and \code{\_relayWindow} is 10 seconds; once a neighbour's window count is exhausted, further relays on its behalf are refused until the window rolls over. Three details govern how that headline number is actually spent, and all three are worth knowing before anyone tunes it:

\begin{itemize}
\item \textbf{The whole budget is spent, and exactly one unit per packet.} \code{\_allowRelayFrom} both tests \emph{and} increments the counter, so it must be called once per relay decision and its answer reused. The relay path used to evaluate it in two arms of the same condition chain --- once in the guard that traces a rate-limited drop, once in the condition that actually relays --- which charged every relayed packet two units and silently halved the ceiling to 256. The call is now hoisted into a single \code{relayAllowed} local that every arm reads, so the effective cap is the constant: \textbf{512 relays per neighbour per 10\,s}.
\item \textbf{Budgets are keyed by peer identity, not by path.} The map key is the neighbour's pubkey hex, resolved from the inbound path, and falls back to the raw path id only while that path is still unauthenticated. Keying by path id alone leaked the budget two ways: a converged dual-leg pair held \emph{two} independent budgets --- one per leg --- and the OS rotates the radio MAC roughly every 30\,s, which minted a fresh key and with it a fresh 512-packet allowance several times per window. The cap is meant to bound a \emph{neighbour}, and a neighbour is an identity.
\item \textbf{Unattributable inbound is uncapped.} \code{\_allowRelayFrom} returns \code{true} immediately when the inbound peer id is \code{null}, which is the UDP case. So the cap governs BLE neighbours, where flooding actually happens; UDP relays are not per-neighbour rate-limited at all. In practice the Internet transport is direct point-to-point and this branch rarely relays, but it is a gap rather than a designed safeguard.
\end{itemize}
...
Buffered packets move between nodes through exactly one mechanism: a sealed \emph{sync exchange} that runs when a pairing forms. The holder offers the packet IDs it carries; the peer answers with the subset its own seen-set lacks; the holder conveys exactly those. The peer \emph{always} answers, sending an \textbf{empty} request when it wants nothing (\code{\_handleSyncOffer}, \code{message\_router.dart}). The reply is mandatory precisely so that it cannot be confused with its absence: were a peer allowed to stay quiet when it wanted nothing, the offerer would have no way to tell ``I already hold all of these'' from ``I never heard you'' or ``I am gone,'' and would keep re-offering the same ids forever. Making the answer obligatory inverts that --- silence stops being ambiguous and becomes evidence in its own right, meaning the exchange did \emph{not} complete, which is exactly what the incomplete-round rule below keys on. Both control frames are \code{SecureFrame}s sealed to the pair's Noise session (\code{ContentType.syncOffer}, \code{syncRequest}) --- the session already exists by the time sync runs, so there is no reason to put a list of buffer contents on the air in the clear.

That guaranteed reply is what lets an offer stop repeating. The offerer keeps a per-peer \textbf{decline ledger}: when a round closes, every id it offered and was not asked for is recorded against that peer (\code{\_declinedByPeer}, \code{message\_router.dart}), and \code{buildSyncOffers} filters those ids out of every later offer to it. Without the ledger an offer is pure repetition --- a carrier holding $N$ packets re-lists all $N$ on every announce cycle to a neighbour that has already refused them, spending $\lceil N/8 \rceil$ sealed packets to learn nothing; with it, a converged pair falls silent and the steady state costs zero packets. Two guards stop the ledger from suppressing a real delivery. First, \textbf{silence is not a decline}: an offer rides an unacknowledged notification, so the peer can walk out of range, lose a chunk on the air, or be gone before its reply is written --- and if fewer requests come back than chunks were actually put on the link, the round declines \emph{nothing} and the whole set is offered again. The asymmetry is deliberate: being wrong that way costs one repeated offer, while being wrong the other way strands the packets until age expiry, because Noise sessions survive link loss and nothing would ever run to undo the mistaken decline. Second, a \textbf{new Noise session} with a peer clears its ledger (\code{clearSyncDeclines}), since a peer that restarted lost its seen-set along with its session and its old refusals no longer describe it.
...
The cost was measured over a 90-cycle overnight run. At \emph{every} reconnection the receiving phone re-sent \textbf{30.9 packets} the other side had already dealt with: 2784 wasted packets across the run. They were all acknowledgements --- the receiver had ACKed each message and kept its copy, and because nothing acknowledges an acknowledgement those copies never left the store, so the whole accumulated set went back on the air each time the link re-formed. At 187 bytes per ACK that is roughly 520\,KB out of 1686\,KB of acknowledgement traffic: \textbf{31\% of it was redundant}. Offering identifiers instead is far cheaper, and the arithmetic is fixed by the sync codec rather than estimated: a sync payload is \code{[count:1][packetId:16] $\times$ count}, and one sealed write holds \code{maxSyncIdsPerPacket} $= (244 - 100 - 1) \div 16 = \textbf{8}$ ids. Offering 32 ids therefore costs four sealed writes of 100 bytes of framing --- the 54-byte packet header, the 25-byte Noise seal and the 21-byte frame header --- plus 129 bytes of payload, so \textbf{916 bytes} --- against roughly 6\,KB to send the 32 packets themselves. Larger sets are chunked into further self-contained packets; sync packets are neighbour-local and single-hop, so each chunk is acted on independently and there is no reassembly.
...
The bound applies to every buffer on the message path, not just the sealed store. The \textbf{pre-seal hold} --- messages waiting for a first session, which still retain full plaintext payloads --- is capped at 256, with the oldest evicted and surfaced as failed rather than vanishing. The \textbf{packet-ID index}, which maps a message to the buffered packets to release when its ACK arrives, is capped at \code{MessageRouter.dtnCapacity} --- i.e.\ the DTN store's own \code{maxTotal} of \textbf{8192} --- because an index entry stops being useful exactly when its last packet leaves the buffer, and at most that many packets can be live at once. The number was corrected for cause: the earlier bound of 1000, borrowed from \code{MessagesState.maxMessages} (an unrelated UI-history depth), sat 8$\times$ under the buffer and filled in about 50\,s at the measured saturation rate. A 7-device smoke run evicted 178{,}138 index entries, which silently turned ACK-driven buffer release off and left packets held until they aged out. Capping the sealed buffer while an unsealed one in front of it grows without limit merely moves the leak upstream --- and sizing a cap against the wrong quantity breaks the mechanism it was meant to protect. The sync exchange's own bookkeeping obeys the same rule. The per-peer decline ledger is pruned to the buffer's live ids on every offer build --- an id that has left the store can never be offered again, so keeping its decline would only grow the set --- which bounds it by the buffer rather than by every packet ever held; on top of that it is capped at 2048 ids per peer across at most 64 peers. The open-round table is capped identically, because a round holds a set the size of the buffer and a stranger offered to once and never met again would otherwise pin it for the life of the process.
...
Two of the eight \code{ContentType} values exist for this subsystem, and they are the reason it touches the wire at all. \code{testbedStart} broadcasts a run start: a field run puts phones hundreds of metres apart, where tapping each in sequence skews their timelines by however long the walk takes and smears the mesh composition at every step boundary, so one tap starts every armed peer and collapses that skew to the flood's own latency. \code{testbedNeighbours} gossips each phone's one-hop neighbour list while the runner sits armed, so the operator can take the connected component containing self and answer the question a single device cannot --- is every phone reachable, or is one stranded behind a gap --- before pressing start. It is armed-only and never runs during a measurement, because control traffic on the air would corrupt the throughput it is there to protect.
...
\begin{enumerate}
  ...
  \item \emph{Park only when there is neither a session nor a peer record.}
    Sealing needs a session, and the handshake that would produce one needs a
    peer record to dial, so \code{shouldParkUnsealedSend} parks the plaintext
    only when \emph{both} are missing. A recipient that has merely gone quiet
    is still sealed and buffered: the stale sweep deletes its peer record after
    two silent announce cycles while its Noise session survives untouched, and
    requiring a record here was measured on 2026-08-10 to strand every send to
    an absent recipient --- 34 sends, every one parked, none buffered, so no
    relay could carry them and no sync exchange could ever offer them,
    precisely inverting what store-carry-forward exists to do. When it does
    park, \code{send} still returns the
...
\begin{enumerate}
  ...
  \item \emph{Reconcile on the next pairing.} Nothing is pushed when a peer
    ...
    the ones its seen-set lacks, and only those are conveyed. A peer that wants
    nothing still \emph{replies}, with an empty \code{syncRequest}: silence and
    ``I already have all of these'' are indistinguishable to the offerer, and
    it is the explicit empty reply that lets the offerer stop. Each node keeps
    a per-peer \emph{decline ledger} --- the ids it offered that the peer did
    not ask for --- and filters them out of every later offer, so a converged
    pair's steady state costs zero sync packets instead of re-offering the
    whole buffer on every announce cycle. Declines are taken only from a
    \emph{complete} exchange (every offer chunk answered): silence is not a
    decline, and because Noise sessions outlive the link, mistaking a lost
    chunk for ``it has them'' would suppress those ids until age expiry. The
    ledger is dropped by \code{clearSyncDeclines} on a new Noise session, which
    is the signal that the peer's seen-set may have reset.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \code{lib/src/proto/bitchat.proto} survives from the Bitchat ancestry, referenced by nothing. It is dead weight rather than a compatibility shim, but it is the last artefact of a lineage the repository otherwise finished retiring.
