...
The remainder of this chapter builds outward from these foundations. \emph{Identity, Trust \& the Security Model} develops the key-pair identity, the sender-anonymous envelope, and why relaying is unauthenticated by construction. \emph{Wire Format, Packets \& the Data Model} specifies the 60-byte header and the sealed \code{SecureFrame} that now carries content type and fragmentation inside the ciphertext. \emph{Transport Abstraction \& the BLE Mesh Transport} covers the common interface and the mandatory dual-role BLE design, and \emph{The Internet Transport \& NAT Traversal} covers the UDX path, address candidates, and hole-punching. \emph{Mesh Routing} details request-driven conveyance, the seen-packetId set (\code{SeenPackets}) that deduplicates it, the DTN memory buffer that makes store-carry-forward work, and its sealed GCS sync exchange. \emph{The Noise Session Layer} covers the \code{Noise_XX} handshake, trial-decrypt, and the AAD construction. \emph{State Management} explains the Redux projection, and \emph{Instrumentation} describes the tracing and testbed machinery that produces the measured numbers this chapter relies on. Later sections walk through end-to-end interactions and close with design decisions, trade-offs, and future work.
...
Where timeout reasoning does live is one layer up, and it is worth being exact rather than claiming the system contains no such timer. A UDP disconnect drops reachability immediately when an explicit disconnect event arrives --- that is the primary path. \emph{In addition}, the coordinator runs a periodic stale sweep: \code{computeStaleUdpPeerPubkeys} flags any UDP-connected peer whose \code{lastUdpSeen} is older than ten announce intervals, and \code{_removeStalePeers} acts on the result (\code{lib/src/grassroots_network.dart}). That is a silence-based inference, deliberately sited in the coordinator --- which owns transport facts and may derive from them --- rather than in a reducer, which may not. The invariant constrains \emph{who} may infer; it does not claim that nothing anywhere reasons from silence.
