...
Signaling is the out-of-band control channel that lets two NAT'd peers rendezvous. Its messages are defined by \code{SignalingType} in \code{lib/src/signaling/signaling_codec.dart}, an enum whose wire values \emph{start at 0x05}: \code{punchInitiate(0x05)}, \code{punchReady(0x06)}, \code{addrReflect(0x07)}, \code{reconnect(0x08)}, \code{friendList(0x0b)}, \code{introduce(0x0c)}. The gaps are permanent and they are a readable fossil record of two clean breaks: \code{0x00}--\code{0x04} were \code{ADDR\_QUERY}/\code{ADDR\_RESPONSE}/\code{PUNCH\_REQUEST}, deleted when the flow switched to \code{RECONNECT} mediation, and \code{0x09}--\code{0x0a} were \code{AVAILABLE}/\code{RV\_LIST}, which died with the rendezvous servers. Per the no-legacy rule the numbers are never reused.
...
Crucially, a relay forwards the packet's bytes \emph{unchanged}. The router hands the decremented packet to the coordinator's \code{onRelay} callback (\code{grassroots\_network.dart:3894}), which re-floods over BLE only --- UDP stays direct point-to-point, per \emph{The Internet Transport} --- via \code{\_floodViaBle(packet.serialize())} and thence \code{BleTransportService.broadcast(bytes, excludePeerIds)}, writing that one identical, already-sealed ciphertext to every ready BLE neighbour. A relay \emph{cannot} re-seal, because it cannot decrypt; it copies opaque bytes. This is distinct from the coordinator's own \code{broadcast(payload)} method, which does seal a \emph{fresh} copy per neighbour under each peer's Noise session --- but that path sends plaintext the coordinator itself holds, and is not relaying. On the relay path the inbound peer is excluded from the fan-out (\code{excludeBlePeerId}, \code{message\_router.dart:210}) so a packet is never echoed back at the neighbour it arrived from. That exclusion is resolved to a peer \emph{identity}, not to a path id: a converged pair holds two GATT legs, and excluding only the leg a packet arrived on let the pair's other leg echo it straight back.

For the same reason, a flood writes each packet to exactly \textbf{one leg per peer}, preferring our peripheral leg --- a notification is unacknowledged at ATT level and several can pack into one connection interval, whereas the central leg writes to the peer's characteristic. Writing both legs put identical bytes on the air twice per neighbour and the receiver's packetId Bloom simply dropped the second copy. This was measured rather than reasoned about: a 90-cycle soak recorded exactly 1000 redundant packet arrivals for 500 delivered messages before the change, and 0 after. The second leg is still maintained --- it is the pair's other direction and its redundancy on failure --- it just does not carry a duplicate of the same packet.
...
\begin{itemize}
\item \textbf{Age.} Entries older than 6~hours are pruned on every access.
\item \textbf{Total packets.} At most 8192 across all recipients, evicting the globally oldest. This replaced a per-recipient depth of 32, which had two faults: it silently dropped a busy peer's oldest undelivered packets while the rest of the buffer sat empty, and --- because confirmations only age out --- it pinned a full 32 held acknowledgements per peer, which is what made the blind push cost exactly that much.
...
The bound applies to every buffer on the message path, not just the sealed store. The \textbf{pre-seal hold} --- messages waiting for a first session, which still retain full plaintext payloads --- is capped at 256, with the oldest evicted and surfaced as failed rather than vanishing. The \textbf{packet-ID index}, which maps a message to the buffered packets to release when its ACK arrives, is capped at 1000 because entries leave only on that ACK. Capping the sealed buffer while an unsealed one in front of it grows without limit merely moves the leak upstream.
...
\begin{itemize}
  ...
  \item \textbf{SettingsState} (\code{lib/src/store/settings_state.dart}) holds the user-toggleable configuration: \code{bluetoothEnabled} and \code{udpEnabled} (both default true), \code{transportPriority} (default $[\text{bluetooth},\text{udp}]$), \code{coldCallTrustLevel} (default \code{closed}), \code{facilitateInvites}, the debug \code{bleRoleMode} and \code{showLinkDiagnostics}, and the testbed-only \code{bulkFlowConfig} (null and inert outside an experiment).
...
\begin{enumerate}
  ...
  \item \emph{Buffer them.} The same sealed packets go into the sender's own
    DTN memory buffer, keyed by recipient, with their IDs indexed under the
    \code{messageId} --- the sender holds its own outgoing packets exactly as a
    relay would hold a stranger's. There is no watchdog and no timeout: an
  ...
  \item \emph{Hold when there is no session.} If no Noise session with the
    recipient exists yet, nothing can be sealed. \code{send} still returns the
    \code{messageId} and the plaintext waits in the bounded pre-seal hold;
    the eager handshake on the next accepted pairing seals it, buffers it,
    and floods --- see \code{\_sealPendingFor}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \emph{If for us: trial-decrypt and deliver.} A session-encrypted packet
    ...
    signaling, syncOffer, syncRequest). Delivery to the application happens
    ...
    That inner-id dedup is distinct from the outer-\code{packetId} relay
  ...
  \item \emph{Buffer on miss.} If, after the relay decision, the recipient
    is \emph{not} currently reachable, the sealed packet is cached in the
    bounded \code{DtnStore} (256 recipients, 8192 packets in total, 6\,h max
    age) keyed by recipient hex --- store-carry-forward for a peer out of range.
  \item \emph{Reconcile on the next pairing.} Nothing is pushed when a peer
    appears. Once a Noise session forms, the two sides run the sealed sync
    exchange: this node offers the packet IDs it carries, the peer requests
    the ones its seen-set lacks, and only those are conveyed. A peer that
    already holds a packet asks for nothing.
\end{enumerate}
...
At 128 sessions the phone saturates a core at 22 transit packets per second --- below the 34--57 messages per second a single saturated BLE link already delivers (\emph{Throughput}). It cannot keep pace with one busy neighbour, let alone the seven concurrent links the hardware supports.
...
Store-carry-forward (DTN) lets a node hold a packet whose recipient is out of range and convey it when that recipient is reachable again. Left unbounded this is an obvious battery sink and abuse vector, so the cache is capped on three axes: \code{DtnStore} holds at most \code{maxRecipients} = 256 recipients and \code{maxTotal} = 8192 packets in total, expiring after \code{maxAge} = 6 hours (\code{lib/src/mesh/dtn_store.dart}). Caching happens only inside the relay branch after a successful relay decision; if the per-neighbour budget already blocked the relay, the packet is not cached either.

The store-wide total replaced an earlier per-recipient depth of 32, and the reason is instructive. A per-recipient cap looks fairer but is not: it dropped a busy peer's oldest undelivered packets while the rest of the buffer sat empty, and because confirmations leave only by age expiry it pinned a full 32 held acknowledgements per peer --- which is precisely what made the (since-deleted) blind push on connect cost about 32 redundant packets every reconnection. There is deliberately no separate sender-side queue to bound alongside it: the sender puts its own packets in the same buffer, so there is one buffer and one number, \code{dtnBufferedCount}.
