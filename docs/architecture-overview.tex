...
\subsection{Decided but not built: conveyance over UDX, targeted only}

\textbf{This subsection records a decision for future work. The code today does not do any of it}, and nothing else in this chapter should be read as describing it as shipped. It is written down here because the reasoning is settled and the alternative readings of it are wrong in instructive ways.

\emph{What the code does today.} Conveyance out of the DTN buffer is BLE-only, and the restriction is structural rather than a policy check in one place. It is enforced twice. On the sending side, the coordinator's \code{onSyncSend} callback resolves the BLE service and, if there is none, traces a \code{sync}/\code{conveyNoTransport} drop and returns --- there is no other transport it could hand the packet to (\code{lib/src/grassroots\_network.dart}). And the handle it is given cannot name one: a \code{SyncLink} carries the authenticated peer pubkey plus a \code{bleDeviceId}, and that BLE device id is its only transport address (\code{lib/src/routing/message\_router.dart}). On the receiving side the same rule is applied to the trigger: a \code{ContentType.syncFilter} frame is dispatched to \code{\_handleSyncFilter} only under the gate \code{transport == PeerTransport.bleDirect}, so a sync filter that arrives over the Internet transport is decoded, authenticated, and then ignored (\code{message\_router.dart}).

\emph{The decision.} Allow conveyance over UDX, but convey \emph{only packets addressed to that peer} --- never third-party transit traffic. Mechanically this is a smaller operation than the BLE path, not a larger one: instead of diffing the whole buffer against a GCS filter, the responder looks up the single bucket \code{DtnStore.\_byRecipient[peerHex]} (the existing \code{packetsFor} accessor). That bucket is by construction a subset of the whole store --- equal to it only in the degenerate case where everything held happens to be addressed to that one peer, the empty buffer included --- so the Internet path can never move more than the BLE path would, and normally moves far less.

\emph{Why the restriction, and what it is not about.} The restriction is about \textbf{volume, not trust}. On BLE the epidemic is self-limiting by physics: encounters are sporadic, airtime is scarce, and a filter round advertises at most \code{maxElements} = 106 ids per sealed frame. Over UDX neither brake exists --- connections are continuous and fast --- so unrestricted conveyance would replicate a bounded-but-large buffer (\code{maxBytes} = 512\,MiB) toward every connected friend at link speed, and any well-connected node would steadily accumulate everyone else's backlog. That node then \emph{is} infrastructure: an always-on store-and-forward relay with a privileged view of who is holding traffic for whom, which is exactly the thing the rendezvous-server removal was meant to eliminate (\emph{Facilitators}). Targeting the peer's own bucket removes the accumulation without touching the trust model.
...
\begin{itemize}
  ...
  \item \textbf{It needs no new trigger.} ANNOUNCE is already broadcast over the Internet transport today: the single \code{\_announceTimer}, running at \code{config.announceInterval}, fires \code{\_broadcastAnnounce()} for BLE and \code{\_broadcastAnnounceViaUdp()} --- ending in \code{await \_udpService!.broadcast(announce)} --- on the same tick (\code{lib/src/grassroots\_network.dart}). The announce cycle that the sync exchange already rides therefore exists on UDX right now. The change is the deletion of a transport gate, not the addition of machinery.
\end{itemize}

\emph{Consequences.} Age expiry is \textbf{unchanged}: \code{maxAge} = 6 hours for every recipient, with no friend exception and no per-transport variant. What changes is which deadline usually wins. A message addressed to a friend reachable only over the Internet leaves the buffer on the next announce cycle instead of waiting for a BLE encounter, so reaching the age bound becomes the rare outcome rather than the likely one --- which is the whole point, since that case is precisely the one the current BLE-only rule serves worst. Reach also starts to follow the social graph: a packet held by C for R is conveyed when C shares a connection with R, so an $A \rightarrow C \rightarrow R$ path works whenever C is connected to both, and the same composition extends to friend-of-friend chains under the ordinary TTL bound. Note what does \emph{not} follow: this is still not third-party relaying over the Internet, because C only ever hands R packets addressed to R.

Two bookkeeping notes. First, the project invariant currently reads that the Internet transport moves nothing out of the buffer; adopting this decision \emph{removes an exception} rather than introducing a rule --- ``convey held packets to the peer they are addressed to'' would simply become transport-independent, and only third-party transit would remain BLE-only. Second, no measurement is invalidated: the Internet transport is switched off on the phones for every experiment run --- BLE is the whole subject --- so nothing in the recorded results depends on which side of this decision the code sits.
