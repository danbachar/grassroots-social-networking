...
| `dial-1-parallel-burst-ladder` | **Parallel dial probe**: DUT rotates 1..7, passive peers never initiate; bursts of N=1..6 simultaneous central dials x 10 reps, 20 s deadline. Formation clock burst -> GATT-usable (Noise as bonus). Per-device score card: P* (largest N with every dial formed in time), median ms at P*, serialization slope. | `09d54cf` |