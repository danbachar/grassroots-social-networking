...
class _TestbedScreenState extends State<TestbedScreen> {
  ...
  void dispose() {
    ...
    _sweepDwellController.dispose();
    _sweepSendsController.dispose();
  ...
  List<Widget> _autoRunnerSection() {
    return [
      ...
      const Text(
        ...
        'step marker on your IN POSITION tap, holds the dwell, then marks '
        'end, settles, stops, and uploads.',
        style: TextStyle(fontSize: 13, color: Colors.grey),
      ),
      const SizedBox(height: 8),
      Row(children: [
      ...
      if (_selectedPreset == FieldPlanPresets.lineSweepPresetName) ...[
        ...
        Row(children: [
          Expanded(
            child: TextField(
              controller: _sweepDwellController,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(
                border: OutlineInputBorder(),
                isDense: true,
                labelText: 'Dwell (s per trial)',
              ),
              onChanged: (text) {
                final v = int.tryParse(text.trim());
                if (v == null || v < 5) return;
                setState(() => _sweepDwellSec = v);
                _applySweep();
              },
            ),
          ),
          ...
          Expanded(
            child: TextField(
              controller: _sweepSendsController,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(
                border: OutlineInputBorder(),
                isDense: true,
                labelText: 'Messages per trial',
              ),
              onChanged: (text) {
                final v = int.tryParse(text.trim());
                if (v == null || v < 1) return;
                setState(() => _sweepSendCount = v);
                _applySweep();
              },
            ),
          ),
        ...
        Text(
          ...
          '$_sweepSendCount messages per trial each way over '
          '${_sweepDwellSec}s '
          '(${(_sweepSendCount / _sweepDwellSec).toStringAsFixed(1)}/s, '
          '${_sweepSendCount * _sweepTrials} a position), '
          'total ${_sweepSendCount * _sweepTrials * _sweepPositions} each way',
      ...
      TextField(
        ...
        decoration: const InputDecoration(
          border: OutlineInputBorder(),
          hintText: 'FieldPlan JSON — pick a preset or paste',
        ),
      ),
      if (_planError != null) ...[
        const SizedBox(height: 6),
  ...
  int _sweepTrials = 10;
  int _sweepDwellSec = 30;
  int _sweepSendCount = 100;
  String? _selectedPreset;

  late final TextEditingController _sweepReceiverController =
      TextEditingController();
  late final TextEditingController _sweepDwellController =
      TextEditingController(text: '$_sweepDwellSec');
  late final TextEditingController _sweepSendsController =
      TextEditingController(text: '$_sweepSendCount');
  ...
  FieldPlan get _sweepPlan => FieldPlanPresets.lineSweepUpTo(
        ...
        trials: _sweepTrials,
        dwellSec: _sweepDwellSec,
        sendCount: _sweepSendCount,
