...
void main() {
  group('pickRawPath', () {
    final peer = _pubkey(0);
    final peerHex =
        peer.map((b) => b.toRadixString(16).padLeft(2, '0')).join();
    final lookup = _lookup({
      'central-A': _pubkey(0),
      'peripheral-A': _pubkey(0),
      'central-B': _pubkey(100), // another peer — must never be picked
    });

    test('notify picks our peripheral leg, write our central leg', () {
      final ready = [
        _path('central-A', ble.BleRole.central),
        _path('peripheral-A', ble.BleRole.peripheral),
        _path('central-B', ble.BleRole.central),
      ];
      expect(
          BleTransportService.pickRawPath(
                  ready: ready, pubkeyFor: lookup, peerHex: peerHex,
                  leg: 'notify', seq: 0)!
              .pathId,
          'peripheral-A');
      expect(
    ...
    test('stripe alternates legs by seq parity', () {
      final ready = [
        _path('central-A', ble.BleRole.central),
        _path('peripheral-A', ble.BleRole.peripheral),
      ];
      ...
      expect(picks,
          ['peripheral-A', 'central-A', 'peripheral-A', 'central-A']);
    });

    test('stripe degrades to the one existing leg instead of stalling', () {
      final ready = [_path('peripheral-A', ble.BleRole.peripheral)];
    ...
    test('a missing leg or unknown mode returns null', () {
      final ready = [_path('peripheral-A', ble.BleRole.peripheral)];
      expect(
          BleTransportService.pickRawPath(
              ready: ready, pubkeyFor: lookup, peerHex: peerHex,
