...
void main() {
  ...
  test('a relay record carries the rx->tx dwell on one clock', () async {
    ...
    group('dispatchOutbound', () {
      ...
      test('buffers and reports not-aired when no direct leg exists', () async {
        final before = router.dtnBufferedCount;
        router.directSend = (pubkey, sealed) async => false;
        final aired = await router.dispatchOutbound(
            otherPubkey, sealedTo(otherPubkey));
        expect(aired, isFalse);
      ...
      test('writes directly and does NOT buffer when the recipient is '
        ...
        final aired = await router.dispatchOutbound(otherPubkey, packet);
        expect(aired, isTrue);
